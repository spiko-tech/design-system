...
const spikoAlertVariants = cva(
  "flex items-center gap-4 rounded-md px-4 py-3 spiko-text-sm-regular",
  {
    variants: {
      variant: {
...
export const SpikoAlert = ({
  ...
  children,
  variant,
}: { className?: string; children: React.ReactNode } & VariantProps<
  typeof spikoAlertVariants
>) => {
  const IconComponent = variantIcons[variant ?? defaultVariant];

  return (
    <div
      className={cn(spikoAlertVariants({ variant }), className)}
      role="alert"
      aria-live="polite"
    >
