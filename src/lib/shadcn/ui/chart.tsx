...
const ChartTooltipContent = ({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return <div className={cn('spiko-text-base-medium', labelClassName)}>{labelFormatter(value, payload)}</div>;
    }
    ...
    return <div className={cn('spiko-text-base-medium', labelClassName)}>{value}</div>;
  ...
  return (
    <div
      className={cn(
        'shadow-xl grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs',
        className
      )}
    >
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  {itemConfig?.icon ? (
                    ...
                    !hideIndicator && (
                      <div
                        ...
                        style={
                          { '--color-bg': indicatorColor, '--color-border': indicatorColor } as React.CSSProperties
                        }
                      />
                    )
                  )}
                  ...
                  >
                    ...
                    {item.value && (
                      <span className="spiko-text-base-medium font-mono text-foreground tabular-nums">
